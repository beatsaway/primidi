...
scene.background = null; // Transparent so body's dark green gradient shows through
...
// Single shared AudioContext for the whole app (synths, filter, effects) to avoid
...
let synth = null;
let additiveSynth = null;  // Kept when switching to Piano Calm (sample-based) preset
let samplePianoSynth = null;

function showPianoCalmUnavailable() {
    var msg = document.getElementById('piano-calm-unavailable-msg');
    if (msg) return;
    msg = document.createElement('div');
    msg.id = 'piano-calm-unavailable-msg';
    msg.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.85);color:#fff;padding:12px 20px;border-radius:8px;font-family:Inter,sans-serif;font-size:14px;z-index:9999;max-width:90%;text-align:center;';
    msg.textContent = 'Piano Calm samples missing. Using default synth. Add WAV files to sound/piano_calm_wavs/ to enable Piano Calm.';
    document.body.appendChild(msg);
    setTimeout(function () { if (msg.parentNode) msg.parentNode.removeChild(msg); }, 6000);
}
if (typeof window !== 'undefined') window.showPianoCalmUnavailable = showPianoCalmUnavailable;

function initializeSynth() {
    if (additiveSynth) {
        synth = additiveSynth;
        if (typeof window !== 'undefined') window.synth = synth;
        return synth;
    }
    if (!sharedAudioContext) {
        sharedAudioContext = new (window.AudioContext || window.webkitAudioContext)();
    }
    additiveSynth = new PolySynthWrapper({
        maxPolyphony: 64,
...
// Filter uses shared context so it works with both additive and sample piano
let filter = null;
function initializeDynamicFilter() {
    let audioCtx = sharedAudioContext || (synth && synth.synth && synth.synth.audioCtx ? synth.synth.audioCtx : null);
    if (audioCtx && !sharedAudioContext) sharedAudioContext = audioCtx;
    if (!audioCtx) return filter;
    if (!filter) {
...
window.reconnectAudioChain = function() {
    ...
    // Piano Calm (sample piano): direct path only (no filter, no reverb/delay) so level is full
    if (synth === samplePianoSynth) {
        if (synth.synth && synth.synth.audioCtx) {
            try {
                currentOutput.connect(synth.synth.audioCtx.destination);
            } catch (e) {
                console.warn('Piano Calm audio connect failed:', e);
            }
        }
        reverbOutput = null;
    ...
    // Connect through dynamic filter if enabled (additive synth only)
    if (filter) {
...
// v1_09: no midi-mapping; note on/off go direct to PianoCalmEngine
...
// ========== v1_09: Piano Calm only – no midi-mapping, direct engine ==========
let audioCtxV19 = null;

// Which instrument is active: 'piano' | 'guitar' | synth preset name (warmPad, pluck, etc.)
window.calmInstrument = 'piano';

const SYNTH_PRESETS = ['warmPad', 'pluck', 'organ', 'bell', 'bass', 'brightLead', 'softPiano', 'epiano', 'string'];
function isSynthPreset(which) {
    return SYNTH_PRESETS.indexOf(which) !== -1;
}

function handleNoteOn(midiNote, velocity) {
    if (!audioCtxV19) return;
    pressKey(midiNote, velocity);
    if (window.calmInstrument === 'guitar' && window.GuitarCalmEngine) {
        window.GuitarCalmEngine.noteOn(audioCtxV19, midiNote, velocity || 80);
    } else if (isSynthPreset(window.calmInstrument) && window.SynthCalmEngine) {
        window.SynthCalmEngine.setPreset(window.calmInstrument);
        window.SynthCalmEngine.noteOn(audioCtxV19, midiNote, velocity || 80);
    } else if (window.PianoCalmEngine) {
        window.PianoCalmEngine.noteOn(audioCtxV19, midiNote, velocity || 80);
    }
}
...
function handleNoteOff(midiNote) {
    ...
    releaseKey(midiNote);
    if (window.calmInstrument === 'guitar' && window.GuitarCalmEngine) {
        window.GuitarCalmEngine.noteOff(audioCtxV19, midiNote);
    } else if (isSynthPreset(window.calmInstrument) && window.SynthCalmEngine) {
        window.SynthCalmEngine.noteOff(audioCtxV19, midiNote);
    } else if (window.PianoCalmEngine) {
        window.PianoCalmEngine.noteOff(audioCtxV19, midiNote);
    }
}
...
// Safety function to release all stuck notes (Piano Calm + Guitar Calm)
function releaseAllNotes() {
    if (audioCtxV19 && window.PianoCalmEngine) window.PianoCalmEngine.releaseAll(audioCtxV19);
    if (audioCtxV19 && window.GuitarCalmEngine) window.GuitarCalmEngine.releaseAll(audioCtxV19);
    keyMap.forEach((keyData, midiNote) => {
        if (keyData.isPressed) {
            releaseKey(midiNote);
        }
...
// MIDI Input Module Integration – sustain pedal for Piano Calm + Guitar Calm
function handleControlChange(controller, value) {
    if (controller !== 64) return;
    if (window.calmInstrument === 'guitar' && window.GuitarCalmEngine) window.GuitarCalmEngine.setSustainPedal(value >= 64);
    else if (isSynthPreset(window.calmInstrument) && window.SynthCalmEngine) window.SynthCalmEngine.setSustainPedal(value >= 64);
    else if (window.PianoCalmEngine) window.PianoCalmEngine.setSustainPedal(value >= 64);
}

function setCalmInstrument(which) {
    window.calmInstrument = which;
    var t = audioCtxV19 ? audioCtxV19.currentTime : 0;
    var isPiano = which === 'piano';
    var isGuitar = which === 'guitar';
    var isSynth = isSynthPreset(which);
    if (window.pianoMixGain && window.pianoMixGain.gain) {
        window.pianoMixGain.gain.setTargetAtTime(isPiano ? 1 : 0, t, 0.01);
    }
    if (window.guitarMixGain && window.guitarMixGain.gain) {
        window.guitarMixGain.gain.setTargetAtTime(isGuitar ? 1 : 0, t, 0.01);
    }
    if (window.synthMixGain && window.synthMixGain.gain) {
        window.synthMixGain.gain.setTargetAtTime(isSynth ? 1 : 0, t, 0.01);
    }
    if (isSynth && window.SynthCalmEngine) window.SynthCalmEngine.setPreset(which);
    if (audioCtxV19) {
        if (isPiano && window.GuitarCalmEngine) window.GuitarCalmEngine.releaseAll(audioCtxV19);
        if (isPiano && window.SynthCalmEngine) window.SynthCalmEngine.releaseAll(audioCtxV19);
        if (isGuitar && window.PianoCalmEngine) window.PianoCalmEngine.releaseAll(audioCtxV19);
        if (isGuitar && window.SynthCalmEngine) window.SynthCalmEngine.releaseAll(audioCtxV19);
        if (isSynth && window.PianoCalmEngine) window.PianoCalmEngine.releaseAll(audioCtxV19);
        if (isSynth && window.GuitarCalmEngine) window.GuitarCalmEngine.releaseAll(audioCtxV19);
    }
}
if (typeof window !== 'undefined') window.setCalmInstrument = setCalmInstrument;
...
document.addEventListener('click', () => {
    // Piano Calm + Guitar Calm – init AudioContext and both engines on first click
    if (!audioCtxV19) {
        audioCtxV19 = new (window.AudioContext || window.webkitAudioContext)();
        var baseUrl = (typeof document !== 'undefined' && document.baseURI) ? document.baseURI : (typeof window !== 'undefined' && window.location && window.location.href) ? window.location.href : '';
        var loadPiano = window.PianoCalmEngine.ensureLoaded(audioCtxV19, baseUrl);
        var loadGuitar = window.GuitarCalmEngine ? window.GuitarCalmEngine.ensureLoaded(audioCtxV19, baseUrl) : Promise.resolve();
        var loadSynth = window.SynthCalmEngine && window.SynthCalmEngine.ensureLoaded ? window.SynthCalmEngine.ensureLoaded(audioCtxV19, baseUrl) : Promise.resolve();
        Promise.all([loadPiano, loadGuitar, loadSynth]).then(function () {
            if (audioCtxV19.state !== 'running') audioCtxV19.resume();
            var mixer = audioCtxV19.createGain();
            mixer.gain.value = 1;
            var mixPiano = audioCtxV19.createGain();
            mixPiano.gain.value = 1;
            var mixGuitar = audioCtxV19.createGain();
            mixGuitar.gain.value = 0;
            var mixSynth = audioCtxV19.createGain();
            mixSynth.gain.value = 0;
            window.pianoMixGain = mixPiano;
            window.guitarMixGain = mixGuitar;
            window.synthMixGain = mixSynth;
            window.PianoCalmEngine.getMasterGain().connect(mixPiano);
            mixPiano.connect(mixer);
            if (window.GuitarCalmEngine && window.GuitarCalmEngine.getMasterGain()) {
                window.GuitarCalmEngine.getMasterGain().connect(mixGuitar);
                mixGuitar.connect(mixer);
            }
            if (window.SynthCalmEngine && window.SynthCalmEngine.getMasterGain(audioCtxV19)) {
                window.SynthCalmEngine.getMasterGain(audioCtxV19).connect(mixSynth);
                mixSynth.connect(mixer);
            }
            if (window.PianoCalmEffects) {
                window.pianoCalmEffectsRef = window.PianoCalmEffects.connect(audioCtxV19, mixer);
                var volEl = document.getElementById('v19-master-volume');
                if (volEl && window.pianoCalmEffectsRef.setMasterVolume) {
                    window.pianoCalmEffectsRef.setMasterVolume(parseInt(volEl.value, 10) / 100);
                }
                var panEl = document.getElementById('v19-keyboard-pan');
                if (panEl && window.PianoCalmEngine.setKeyboardPan) {
                    window.PianoCalmEngine.setKeyboardPan(parseInt(panEl.value, 10));
                }
                if (window.GuitarCalmEngine && window.GuitarCalmEngine.setKeyboardPan && panEl) {
                    window.GuitarCalmEngine.setKeyboardPan(parseInt(panEl.value, 10));
                }
                if (window.SynthCalmEngine && window.SynthCalmEngine.setKeyboardPan && panEl) {
                    window.SynthCalmEngine.setKeyboardPan(parseInt(panEl.value, 10));
                }
                var instrumentEl = document.getElementById('v19-instrument');
                if (instrumentEl && window.setCalmInstrument) {
                    window.setCalmInstrument(instrumentEl.value);
                }
            } else {
                mixer.connect(audioCtxV19.destination);
            }
            console.log('Piano Calm + Guitar Calm ready');
        }).catch(function (err) {
            console.warn('Sample engine load failed:', err);
        });
        initializeMidiInput();
        initializeKeypressInput();
    }
    if (audioCtxV19 && audioCtxV19.state !== 'running') {
        audioCtxV19.resume();
    }
}, { once: true });
...
const soundPresets = {
    ...
    piano_calm: {
        // Sample-based Piano Calm (v1_64 piano sound); uses SamplePianoWrapper, other settings ignored
        physics: {},
        frequencyCompensation: {},
        delayPseudoReverb: {},
        tremolo: {},
...
function applySoundPreset(presetName) {
    ...
    // Piano Calm: switch to sample-based synth (v1_64 piano sound) if WAVs are present
    if (presetName === 'piano_calm') {
        getSynth(); // ensure additive exists so we can switch back later
        if (typeof window.SamplePianoWrapper === 'undefined') {
            console.warn('SamplePianoWrapper not loaded');
            return;
        }
        if (!samplePianoSynth) samplePianoSynth = new window.SamplePianoWrapper(sharedAudioContext);
        samplePianoSynth.samplesAvailable().then(function (ok) {
            if (!ok) {
                console.warn('Piano Calm: sample WAVs not found in sound/piano_calm_wavs/. Using additive synth.');
                synth = additiveSynth;
                window.synth = synth;
                if (window.Tone) {
                    window.Tone.now = () => synth.synth.audioCtx.currentTime;
                    try { Object.defineProperty(window.Tone, 'context', { get: () => synth.synth.audioCtx, configurable: true }); } catch (e) {}
                }
                if (typeof initializeMidiMapping === 'function') initializeMidiMapping();
                var presetSelect = document.getElementById('preset-select');
                if (presetSelect) presetSelect.value = 'primal';
                updateUIAfterPreset();
                if (window.reconnectAudioChain) window.reconnectAudioChain();
                if (typeof window.showPianoCalmUnavailable === 'function') window.showPianoCalmUnavailable();
                return;
            }
            synth = samplePianoSynth;
            window.synth = synth;
            if (window.Tone) {
                window.Tone.now = () => synth.synth.audioCtx.currentTime;
                try { Object.defineProperty(window.Tone, 'context', { get: () => synth.synth.audioCtx, configurable: true }); } catch (e) {}
            ...
            setTimeout(function () {
                updateUIAfterPreset();
                if (window.reconnectAudioChain) window.reconnectAudioChain();
                if (window.logSettingsDifferences) setTimeout(function () { window.logSettingsDifferences(); }, 100);
            }, 100);
        });
        return;
    }
    // Switch back to additive synth when selecting any other preset
    if (additiveSynth) {
        synth = additiveSynth;
        window.synth = synth;
        if (window.Tone) {
            window.Tone.now = () => synth.synth.audioCtx.currentTime;
            try {
                Object.defineProperty(window.Tone, 'context', { get: () => synth.synth.audioCtx, configurable: true });
            } catch (e) {}
        }
        // So MIDI/keypress use additive synth again
        if (typeof initializeMidiMapping === 'function') initializeMidiMapping();
    }
    ...
    if (presetName === 'primal') {
        setTimeout(() => {
            updateUIAfterPreset();
            if (window.reconnectAudioChain) window.reconnectAudioChain();
            if (window.logSettingsDifferences) setTimeout(() => window.logSettingsDifferences(), 100);
            const presetSelect = document.getElementById('preset-select');
            if (presetSelect && presetSelect.value !== 'primal') presetSelect.value = 'primal';
        }, 300);
        return;
    }
